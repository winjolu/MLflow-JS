...
async function createExperiment(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`HTTP error from tracking server, status: ${response.status}`);
    }
 ...
 * @returns {Promise<Array>} Returns an array of experiment objects matching the filter.
 */
